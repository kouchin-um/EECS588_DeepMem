...
def get_label_vector(output_vector_size, dict_key_node_to_weight=None, obj_type=None, inside_offset=None, node_size=None):
    ...
    if dict_key_node_to_weight != None:
        key_nodes = list(dict_key_node_to_weight.keys())
        key_nodes.sort()
...
def balance_type(lines, list_vector, list_label, dict_key_node_to_vector_list, dict_key_node_to_label_list, dict_key_node_to_idx_list):
    ...
    obj_node_count = 0
    for key_node, label_list in iter(dict_key_node_to_label_list.items):
...
def stat_result_type(dict_addr_to_weight, set_obj_addr, obj_type, dict_key_node_to_weight, threshold=0):
    ...
    best_F1_FPFN_value = ''
    total_weight = 0.0
    for _, weight in iter(dict_key_node_to_weight.items()):
        total_weight += weight
    if threshold == 0:
...
def main():
    ...
    keep_prob = tf.compat.v1.placeholder(tf.float32, name='ph')
    mu = tf.zeros_like(x)
    ...
    y_ = tf.compat.v1.placeholder(tf.float32, [None, output_vector_size], name='y_')
